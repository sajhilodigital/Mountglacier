export interface Destination {
  id: string;
  name: string; // Destination name (e.g., Everest Base Camp Trek)
  location: string; // Location with region/country
  rating: number; // Rating out of 5
  img: string; // Image path or URL
  category: string; // Category (Adventure | Trek | Tour | China | etc.)
  routes: string[]; // Available routes with duration
